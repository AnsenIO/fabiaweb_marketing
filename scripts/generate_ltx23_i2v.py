#!/usr/bin/env python3
"""Generate a FABIABox image-to-video clip using LTX 2.3 image-to-video."""
import argparse
import json
import os
import random
...
WORKFLOW_PATH = os.path.join(os.path.dirname(__file__), "..", "assets", "workflows", "ltx23_i2v.json")
...
CROP_NAME = "fabiabox_birthday_ltx_input.jpg"
...
def crop_to_ratio(input_path, out_path, width=960, height=544):
...
def main():
    ...
    crop_to_ratio(input_path, cropped_path)

    workflow = load_workflow()
    workflow["6"]["inputs"]["image"] = CROP_NAME
    workflow["3"]["inputs"]["text"] = args.prompt
    workflow["4"]["inputs"]["text"] = args.negative
    workflow["10"]["inputs"]["noise_seed"] = args.seed

    print(f"Queueing LTX 2.3 i2v (seed {args.seed})...")
